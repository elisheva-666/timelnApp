...
export default function TimerPage() {
  ...
  const [stopStatus, setStopStatus] = useState('submitted');
  ...
  const handleStop = async () => {
    try {
      const result = await stopTimer({
        description: stopDesc || undefined,
        related_commit_ids: stopCommit || undefined,
        status: stopStatus,
      });
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      {!activeTimer ? (
        /* Start Timer */
        <div className="card">
          <h3 className="text-dark-50 font-semibold mb-5 flex items-center gap-2">
            <Timer size={18} className="text-brand-600" /> הפעל טיימר חדש
          </h3>
        ...
        /* Active Timer */
        <div className="card border-green-300 timer-active">
          <div className="text-center py-8">
            <div className="inline-flex items-center gap-2 text-green-700 mb-3 bg-green-50 px-3 py-1.5 rounded-full">
              <Timer size={15} />
              <span className="text-sm font-medium">{activeTimer.project_name}</span>
              {activeTimer.task_name && <span className="text-green-600">· {activeTimer.task_name}</span>}
            </div>
            <p className="font-mono text-6xl font-bold text-dark-50 mt-2 mb-3 tracking-tight">
              {formatElapsed(elapsed)}
            </p>
            {isPaused && (
              <span className="inline-flex items-center gap-1.5 text-amber-600 text-sm bg-amber-50 px-3 py-1 rounded-full border border-amber-200">
                ⏸ מושהה
              </span>
            )}
          </div>
          ...
          {!showStop ? (
            <div className="flex gap-3">
              ...
              <button
                onClick={async () => { await discardTimer(); toast.success('הטיימר בוטל'); }}
                className="p-2.5 text-dark-400 hover:text-red-500 transition-colors rounded-lg hover:bg-red-50"
                title="בטל טיימר">
                <Trash2 size={18} />
              </button>
            </div>
          ) : (
            <div className="space-y-3 border-t border-dark-700 pt-5 mt-2">
              <p className="text-dark-50 font-semibold">עצור ושמור דיווח</p>
              <div>
              ...
              <div>
                <label className="block text-sm text-dark-300 mb-2">שמור בתור</label>
                <div className="flex gap-2">
                  <button type="button"
                    onClick={() => setStopStatus('submitted')}
                    className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-colors ${stopStatus === 'submitted' ? 'bg-brand-600 text-white border-brand-600' : 'bg-white text-dark-400 border-dark-600 hover:border-brand-400'}`}>
                    הגש לאישור
                  </button>
                  <button type="button"
                    onClick={() => setStopStatus('draft')}
                    className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-colors ${stopStatus === 'draft' ? 'bg-slate-600 text-white border-slate-600' : 'bg-white text-dark-400 border-dark-600 hover:border-slate-400'}`}>
                    שמור כטיוטה
                  </button>
                </div>
              </div>
