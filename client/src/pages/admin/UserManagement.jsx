  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex gap-3 pt-2">
        <button type="submit" disabled={loading} className="btn-primary flex-1">{loading ? 'שומר...' : initial?.id ? 'עדכן' : 'צור משתמש'}</button>
...
export default function UserManagement() {
  ...
  const roleLabel = r => ({ admin: 'אדמין', manager: 'מנהל', employee: 'עובד' }[r] || r);
            ...
            {users.map(u => (
              <tr key={u.id} className={`table-row ${!u.is_active ? 'opacity-50' : ''}`}>
                ...
                <td className="px-4 py-3">
                  <span className={`badge ${u.role === 'admin' ? 'bg-purple-900/60 text-purple-300' : u.role === 'manager' ? 'bg-blue-900/60 text-blue-300' : 'bg-dark-600 text-dark-300'}`}>
                    {roleLabel(u.role)}
                  </span>
                </td>
                ...
                <td className="px-4 py-3">
                  <span className={`badge ${u.is_active ? 'bg-green-900/60 text-green-300' : 'bg-dark-600 text-dark-500'}`}>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    ...
                    <button onClick={() => toggleActive(u)}
                      className="p-1.5 text-dark-500 hover:text-yellow-400 hover:bg-dark-700 rounded transition-colors" title={u.is_active ? 'השבת' : 'הפעל'}>
