...
router.get('/my-summary', authenticate, async (req, res) => {
  try {
    ...
    const [todayHours, weekHours, monthHours, byProject, recent] = await Promise.all([
      db.prepare('SELECT COALESCE(SUM(duration_minutes),0) as total FROM time_entries WHERE user_id=? AND date=?').get(userId, today),
      db.prepare('SELECT COALESCE(SUM(duration_minutes),0) as total FROM time_entries WHERE user_id=? AND date>=?').get(userId, weekStartStr),
      db.prepare('SELECT COALESCE(SUM(duration_minutes),0) as total FROM time_entries WHERE user_id=? AND date>=?').get(userId, monthStart),
      db.prepare(`
...
// Manager overview
router.get('/overview', authenticate, requireRole('manager', 'admin'), async (req, res) => {
  try {
    const { date_from, date_to } = req.query;
    const from = date_from || new Date(new Date().setDate(1)).toISOString().split('T')[0];
    const to   = date_to   || new Date().toISOString().split('T')[0];

    const [byUser, byProject, total] = await Promise.all([
      db.prepare(`
...
router.get('/by-user', authenticate, requireRole('manager', 'admin'), async (req, res) => {
  try {
...
router.get('/by-project', authenticate, requireRole('manager', 'admin'), async (req, res) => {
  try {
    const { project_id, date_from, date_to } = req.query;
    const from = date_from || new Date(new Date().setDate(1)).toISOString().split('T')[0];
    const to   = date_to   || new Date().toISOString().split('T')[0];

    const [taskBreakdown, userBreakdown] = await Promise.all([
      db.prepare(`
...
// Anomalies report
router.get('/anomalies', authenticate, requireRole('manager', 'admin'), async (req, res) => {
  try {
    const { date_from, date_to } = req.query;
    const from = date_from || new Date(new Date().setDate(new Date().getDate() - 30)).toISOString().split('T')[0];
    const to   = date_to   || new Date().toISOString().split('T')[0];

    const [longEntries, heavyDays, noEntryUsers, overlaps] = await Promise.all([
      db.prepare(`
...
router.get('/drill/user/:userId', authenticate, requireRole('manager', 'admin'), async (req, res) => {
  try {
    const { date_from, date_to } = req.query;
    const from = date_from || new Date(new Date().setDate(1)).toISOString().split('T')[0];
    const to   = date_to   || new Date().toISOString().split('T')[0];
...
router.get('/drill/project/:projectId', authenticate, requireRole('manager', 'admin'), async (req, res) => {
  try {
    const { date_from, date_to } = req.query;
    const from = date_from || new Date(new Date().setDate(1)).toISOString().split('T')[0];
    const to   = date_to   || new Date().toISOString().split('T')[0];
